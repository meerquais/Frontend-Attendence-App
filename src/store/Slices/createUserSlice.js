...
export const createUserAction = createAsyncThunk("login/auth", async ({email , password,phoneNumber,courseName,fullName,imgUrl },{rejectWithValue, dispatch}) => {
    ...
    try{
        ...
        const response = await axios.post("http://localhost:5000/api/v1/createuser",{email , password, phoneNo : phoneNumber, courseName, name :fullName, picture:imgUrl},{ headers: { Authorization: `Bearer ${token}` } })
